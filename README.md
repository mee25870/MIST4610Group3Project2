...
**Chart #2:** This chart shows the frequency of (?) incidents (Y-axis), separated by the state in which the crime took place (X-axis). In this case, the chart is able to be manipulated for different purposes (to view different crimes) by changing the Y-axis variable to a preferred type of crime (property crime, homicide, etc.) and then individually viewing the crime frequencies of those categories within the different states. The results in a chart like this tend to favor states like California due to their large population.
